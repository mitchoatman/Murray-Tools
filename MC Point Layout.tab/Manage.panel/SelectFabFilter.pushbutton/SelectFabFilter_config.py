...
class PointNumberWindow(Window):
    def __init__(self, default_value, point_numbers, revit_window_handle):
        ...
        self.initialize_components(default_value, point_numbers)
    ...
    def initialize_components(self, default_value, point_numbers):
        grid = Grid()
        self.Content = grid
        ...
        item_height = 20
        listbox_height = item_height * min(15, max(7, len(point_numbers))) + 5
        self.Height = listbox_height + 165

        row_index = 0

        self.label = Label()
        self.label.Content = "Enter TS Point Number:"
    ...
    def on_show_click(self, sender, event):
        selected = self.listbox.SelectedItem
        if not selected:
            TaskDialog.Show("Warning", "Please select a point number from the list.")
            return
...
if value:
    try:
        picked_refs = uidoc.Selection.PickObjects(
            ObjectType.Element,
            "Please select elements to set TS Point Number."
        )
    ...
    t = None
    try:
        t = Transaction(doc, "Set TS Point Number")
        t.Start()
