...
def place_sleeve_at_intersection(pipe, intersection_point, family_symbol, level, existing_sleeves):
    ...
    # Set diameter directly from pipe size
    ...
    new_instance.LookupParameter('Diameter').Set(diameter)
...
def main():
    ...
    with Transaction(doc, 'Place Sleeves at Intersections') as t:
        ...
        # Mode 1: Pre-selected pipe in Floor Plan - Place sleeve at current view level
        if selected_ids.Count > 0 and curview.ViewType == ViewType.FloorPlan:
            ...
            for element_id in selected_ids:
                ...
                if (pipe.Category.Id.IntegerValue == int(BuiltInCategory.OST_FabricationPipework) and 
                    ...
                    intersections = get_pipe_intersections(pipe, current_level)
                    for point in intersections:
                        new_sleeve = place_sleeve_at_intersection(pipe, point, family_symbol, current_level, existing_sleeves)
                        if new_sleeve:
                            placed_count += 1
        ...
        # Mode 4: Floor Plan View without selection - Populate current view level
        elif curview.ViewType == ViewType.FloorPlan:
            current_level = curview.GenLevel
            if not current_level:
                TaskDialog.Show("Error", "No level associated with the current view")
                t.Commit()
                return
            pipes = FilteredElementCollector(doc, curview.Id).OfCategory(BuiltInCategory.OST_FabricationPipework).WhereElementIsNotElementType().ToElements()
            for pipe in pipes:
