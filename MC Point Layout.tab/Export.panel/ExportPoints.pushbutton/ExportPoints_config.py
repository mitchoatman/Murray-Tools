...
def is_fab_pipework(element):
    try:
        return element and element.Category and \
               get_id_value(element.Category.Id) == int(BuiltInCategory.OST_FabricationPipework)
    except:
        return False


def comments_has_export(element):
    if not element:
        return False
    ...
    try:
        ...
        if p and p.HasValue:
            val = p.AsString()
            if val and "EXPORT" in val.upper():
                return True
    except:
        pass
...
def get_fab_pipework_origin(element):
    if not element:
        return None

    # Best for straight fabrication pipework: midpoint of centerline
    try:
    ...
    # Fallback for fittings / parts with 2+ connectors:
    # use midpoint between connectors instead of element.Origin
    try:
    ...
    # Next fallback
    try:
        if hasattr(element, "Origin") and element.Origin:
...
def get_parameter_value_by_name_AsDouble(element, param_name):
    if not element:
        return 0.0
...
def set_parameter_by_name(element, param_name, value):
    if not element:
        return False
...
def update_sleeve_descriptions_in_view():
    pipe_accessories = list(
    ...
    accessory_elements2 = [element for element in pipe_accessories if "Pipe Riser" in element.Name]
    ...
    accessory_elements6 = [element for element in pipe_accessories if "BLOCKOUT" in element.Name]
    ...
    accessory_elements7 = [element for element in duct_accessories if "RWS" in element.Name]
...
class ExportHangerPointsDialog(Window):
    def __init__(self):
        ...
        self.coordinate_system = settings["coordinate_system"]
    ...
    def InitializeComponents(self):
        ...
        self.cb_rck_prefix.IsChecked = self.use_rck_prefix
        ...
        desc_panel = StackPanel()
        desc_panel.Orientation = Orientation.Vertical
        desc_panel.Margin = Thickness(10)
        desc_group.Content = desc_panel
    ...
    def on_make_template_clicked(self, sender, args):
        try:
            template_path = self.txt_filename.Text.strip() if self.txt_filename.Text else self.output_path

            if not template_path:
                MessageBox.Show(
                    "Enter or browse to a CSV file path first.",
                    "Missing File",
                    MessageBoxButton.OK,
            ...
            if not template_path.lower().endswith(".csv"):
                template_path += ".csv"
            ...
            if self.rb_yxz.IsChecked:
                headers = ["POINT NUMBER", "Y", "X", "Z", "DESCRIPTION"]
            else:
                headers = ["POINT NUMBER", "X", "Y", "Z", "DESCRIPTION"]

            with open(template_path, 'wb') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(headers)

            self.output_path = template_path
            self.txt_filename.Text = template_path
...
def perform_export():
    ...
    if dlg.rb_pick.IsChecked:
        try:
            ...
            for el in raw_elements:
                if el and el.Category:
                    if get_id_value(el.Category.Id) == int(BuiltInCategory.OST_FabricationHangers):
                        temp_list.append(el)
                    elif is_fab_pipework(el) and comments_has_export(el):
                        temp_list.append(el)
                    else:
            ...
            seen = set()
            selected_elements = [
                el for el in temp_list
                if get_element_id_value(el) not in seen and not seen.add(get_element_id_value(el))
            ]
        ...
        export_fab_pipework = [el for el in fab_pipework if comments_has_export(el)]

        selected_elements = list(hangers) + gtps + export_fab_pipework
    ...
    for element in element_list:
        ...
        if is_hanger:
            ...
            if is_fab_pipework(element):
                description = "EXPORT PIPE"
                try:
                ...
                try:
                    pos = get_fab_pipework_origin(element)
                    if pos is None:
                        skipped_count += 1
                        continue
