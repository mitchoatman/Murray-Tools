...
family_filename = 'Manual Point.rfa'
family_path = os.path.join(script_dir, family_filename)
family_name = 'Manual Point'

# Last used type storage
folder_name = r"c:\temp"
filepath = os.path.join(folder_name, 'Ribbon_ManualPoint.txt')

if not os.path.exists(folder_name):
    os.makedirs(folder_name)


def read_last_type(txt_path, default_value='WASTE'):
    try:
...
def save_last_type(txt_path, value):
    try:
        with open(txt_path, 'w') as f:
            f.write(value or '')
    except:
...
prev_input = read_last_type(filepath, 'WASTE')
...
# Family/type manager
# --------------------------------------------------
class ManualPointFamilyManager(object):
    ...
    def load_family_if_missing(self):
        ...
        try:
            t = Transaction(self.doc, 'Load Manual Point Family')
            t.Start()
    ...
    def get_or_create_symbol(self, type_name):
        type_name = (type_name or '').strip().upper()
        if not type_name:
            return None

        existing = self.get_symbol_by_type_name(type_name)
        if existing:
            return existing
        ...
        t = Transaction(self.doc, 'Create New Manual Point Type')
        t.Start()
        try:
            ...
            if not param:
                t.RollBack()
                TaskDialog.Show("Error", "Parameter 'TS_Point_Description' not found.")
                return None

            if param.IsReadOnly:
                t.RollBack()
                TaskDialog.Show("Error", "Parameter 'TS_Point_Description' is read-only.")
                return None

            if param.StorageType != DB.StorageType.String:
                t.RollBack()
                TaskDialog.Show("Error", "Parameter 'TS_Point_Description' is not a text parameter.")
                return None

            param.Set(type_name)
            t.Commit()

            self.symbol_cache[type_name] = new_symbol
            return new_symbol

        except Exception as e:
            if t.HasStarted():
                t.RollBack()
            TaskDialog.Show("Error", "Error creating new type '{}': {}".format(type_name, str(e)))
            return None
...
# --------------------------------------------------
# Dialog - same style/behavior as FixtureCL
# --------------------------------------------------
class ManualPointTypeWindow(Window):
    ...
    def InitializeComponents(self):
        ...
        self.label = Label()
        self.label.Content = "Enter Manual Point Type:"
    ...
    def on_place_click(self, sender, event):
        point_type = self.textbox.Text.strip().upper()

        if not point_type:
            TaskDialog.Show("Error", "Please enter a valid Manual Point type.")
            return

        self.selected_type = point_type
