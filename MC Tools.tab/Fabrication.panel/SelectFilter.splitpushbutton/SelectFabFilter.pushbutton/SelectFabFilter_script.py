...
from Autodesk.Revit.DB import FilteredElementCollector, FabricationConfiguration, Transaction, TemporaryViewMode
...
class RemoveFilterDialog(Window):
    ...
    def InitializeComponents(self):
        ...
        panel = StackPanel(Orientation=Orientation.Horizontal, HorizontalAlignment=HorizontalAlignment.Center, Margin=Thickness(0,10,0,10))
        Grid.SetRow(panel, 1)
        grid.Children.Add(panel)
        btn = Button(Content="Remove", Width=80, Height=25, Margin=Thickness(0,0,5,0))
        btn.Click += self.ok_clicked; panel.Children.Add(btn)
        btn = Button(Content="Remove All", Width=80, Height=25, Margin=Thickness(5,0,5,0))
        btn.Click += self.remove_all_clicked; panel.Children.Add(btn)
        btn = Button(Content="Cancel", Width=80, Height=25, Margin=Thickness(5,0,0,0))
        btn.Click += self.cancel_clicked; panel.Children.Add(btn)
...
class MultiPropertyFilterForm(Window):
    ...
    def InitializeComponents(self):
        ...
        grid = Grid()
        self.Content = grid
        heights = [GridLength(40), GridLength.Auto, GridLength.Auto, GridLength(260), GridLength.Auto, GridLength(140), GridLength.Auto]
        for h in heights:
        ...
        grid.ColumnDefinitions.Add(ColumnDefinition(Width=GridLength(1,GridUnitType.Star)))
        self.property_label = Label(Content="Select Property:", Margin=Thickness(10,0,10,0), HorizontalAlignment=HorizontalAlignment.Left)
        Grid.SetRow(self.property_label, 0); Grid.SetColumn(self.property_label,1); grid.Children.Add(self.property_label)
        self.property_combo = ComboBox(Width=160, Height=22, Margin=Thickness(120,0,10,0), HorizontalAlignment=HorizontalAlignment.Left)
        ...
        Grid.SetRow(self.property_combo,0); Grid.SetColumn(self.property_combo,1); grid.Children.Add(self.property_combo)
        self.search_label = Label(Content="Search:", Margin=Thickness(10,0,10,0), HorizontalAlignment=HorizontalAlignment.Left)
        Grid.SetRow(self.search_label,1); Grid.SetColumn(self.search_label,1); grid.Children.Add(self.search_label)
        self.search_box = TextBox(Width=300, Height=20, Margin=Thickness(75,0,10,0), HorizontalAlignment=HorizontalAlignment.Left)
        self.search_box.TextChanged += self.update_values_list
        Grid.SetRow(self.search_box,1); Grid.SetColumn(self.search_box,1); grid.Children.Add(self.search_box)
        self.search_box.Focus()
        self.values_label = Label(Content="Select Values:", Margin=Thickness(10,25,10,0), HorizontalAlignment=HorizontalAlignment.Left)
        Grid.SetRow(self.values_label,2); Grid.SetColumn(self.values_label,1); grid.Children.Add(self.values_label)
        self.values_list = ListBox(Width=500, Height=250, Margin=Thickness(10,0,10,0), SelectionMode=SelectionMode.Extended)
        self.values_list.MouseDoubleClick += self.add_filter
        Grid.SetRow(self.values_list,3); Grid.SetColumn(self.values_list,1); grid.Children.Add(self.values_list)
        self.add_button = Button(Content="Add Filter", Width=80, Height=25, Margin=Thickness(10,20,10,0), HorizontalAlignment=HorizontalAlignment.Right)
        self.add_button.Click += self.add_filter
        Grid.SetRow(self.add_button,2); Grid.SetColumn(self.add_button,1); grid.Children.Add(self.add_button)
        self.filter_button = Button(Width=500, Height=130, Margin=Thickness(10,10,10,0),
        ...
        Grid.SetRow(self.filter_button,5); Grid.SetColumn(self.filter_button,1); grid.Children.Add(self.filter_button)
        self.logic_check = CheckBox(Content="AND logic (unchecked = OR)", Width=230,
                                    Margin=Thickness(10,5,10,0), HorizontalAlignment=HorizontalAlignment.Center)
        Grid.SetRow(self.logic_check,4); Grid.SetColumn(self.logic_check,1); grid.Children.Add(self.logic_check)
        panel = StackPanel(Orientation=Orientation.Horizontal, HorizontalAlignment=HorizontalAlignment.Center, Margin=Thickness(0,15,0,0))
        Grid.SetRow(panel,6); Grid.SetColumn(panel,1); grid.Children.Add(panel)
        for txt, handler in [
            ...
            btn = Button(Content=txt, Width=80, Height=25,
    ...
    def reset_filter_clicked(self, sender, args):
        try:
            ...
            self.fab_elements = pre or FilteredElementCollector(doc,curview.Id).OfClass(DB.FabricationPart).WhereElementIsNotElementType().ToElements()
            self.all_elements = pre or FilteredElementCollector(doc,curview.Id).WhereElementIsNotElementType().ToElements()

            self.property_options = {}
            # fabrication-only list now includes 'Alias'
            fab_props = [
            ...
            for prop in fab_props:
                vals = set(filter(None, [get_property_value(e, prop, False) for e in self.fab_elements]))
                if vals:
                    self.property_options[prop] = sorted(vals)
            # all-elements list (no Alias here)
            all_props = ['Name','Comments','Category','TS_Point_Number','TS_Point_Description']
            for prop in all_props:
                vals = set(filter(None, [get_property_value(e, prop, False) for e in self.all_elements]))
                if vals:
                    self.property_options[prop] = sorted(vals)
        ...
        try:
            ...
            for e in elems:
                if not e or not e.IsValidObject:
                    continue
                ev = {p:get_property_value(e,p,False) for p in self.selected_filters}
                ok = True
            ...
            if ids:
                ...
                t.Start()
                curview.IsolateElementsTemporary(lst)
                t.Commit()
            else:
                dlg = TaskDialog("Warning")
...
fab_elements = preselection or FilteredElementCollector(doc,curview.Id).OfClass(DB.FabricationPart).WhereElementIsNotElementType().ToElements()
all_elements = preselection or FilteredElementCollector(doc,curview.Id).WhereElementIsNotElementType().ToElements()
...
for prop in fab_props:
    vals = set(filter(None,[get_property_value(e,prop,False) for e in fab_elements]))
    if vals:
...
for prop in all_props:
    if all_elements:
        vals = set(filter(None,[get_property_value(e,prop,False) for e in all_elements]))
        if vals:
            property_options[prop] = sorted(vals)
