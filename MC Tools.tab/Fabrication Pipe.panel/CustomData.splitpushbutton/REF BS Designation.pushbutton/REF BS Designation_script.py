...
PARAM_NAME = "FP_REF BS Designation"
FOLDER_NAME = r"C:\Temp"
FILE_PATH = os.path.join(FOLDER_NAME, "Ribbon_REFBSDesignation.txt")
...
def get_refbs_values_in_active_view():
    values = set()
...
def get_elements_by_refbs_value(refbs_value):
    matches = []
    collector = FilteredElementCollector(doc, doc.ActiveView.Id)

    for elem in collector:
        param = elem.LookupParameter(PARAM_NAME)
        if param and param.HasValue and param.AsString() == refbs_value:
            matches.append(elem)
...
class REFBSDesignationWindow(Window):
    def __init__(self, default_value, refbs_values, revit_window_handle):
        Window.__init__(self)

        self.Title = "REF BS Designation"
        self.Width = 300
        ...
        self.InitializeComponents(default_value, refbs_values)
    ...
    def InitializeComponents(self, default_value, refbs_values):
        grid = Grid()
        self.Content = grid
        ...
        item_height = 20
        listbox_height = item_height * min(15, max(7, len(refbs_values))) + 5
        self.Height = listbox_height + 185

        row_index = 0

        self.label = Label()
        self.label.Content = "Enter REF BS Designation:"
        ...
        self.list_label.Content = "REF BS Designations in View:"
        ...
        for value in refbs_values:
            self.listbox.Items.Add(value)
    ...
    def on_listbox_double_click(self, sender, event):
        ...
        self.textbox.Text = str(selected)
        self.show_refbs_value(str(selected))

    def on_show_click(self, sender, event):
        selected = self.listbox.SelectedItem
        if not selected:
            TaskDialog.Show("Warning", "Please select a REF BS Designation from the list.")
            return

        self.textbox.Text = str(selected)
        self.show_refbs_value(str(selected))

    def show_refbs_value(self, refbs_value):
        matching_elements = get_elements_by_refbs_value(refbs_value)

        if not matching_elements:
            TaskDialog.Show(
                "Warning",
                "No elements found with REF BS Designation '{}' in the active view.".format(refbs_value)
            )
            return
...
refbs_values = get_refbs_values_in_active_view()
revit_window_handle = get_revit_window_handle()

form = REFBSDesignationWindow(previous_input, refbs_values, revit_window_handle)
form.Show()
...
if value:
    ...
    if not selected_ids:
        try:
            picked_refs = uidoc.Selection.PickObjects(
                ObjectType.Element,
                "Please select elements to set REF BS Designation."
            )
    ...
    t = None
    try:
        t = Transaction(doc, 'Set REF BS Designation')
        t.Start()
