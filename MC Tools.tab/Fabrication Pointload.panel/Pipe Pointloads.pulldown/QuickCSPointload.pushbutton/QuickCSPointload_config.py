...
servicelist = forms.SelectFromList.show(set(SrvcList), multiselect=True, button_name='Select Service(s) Drawn in CS')
...
for hanger in hanger_collector:
    ...
    try:
        ...
        if HangerSize == '1"':
            set_parameter_by_name(hanger, 'FP_Pointload', 22)
            set_customdata_by_custid(hanger, 7, '22')
        if HangerSize == '1 1/4"':
        ...
        if HangerSize == '2 1/2"':
            ...
            set_customdata_by_custid(hanger, 7, '91')
        if HangerSize == '3"':
            set_parameter_by_name(hanger, 'FP_Pointload', 119)
            set_customdata_by_custid(hanger, 7, '119')
        if HangerSize == '4"':
            set_parameter_by_name(hanger, 'FP_Pointload', 195)
            set_customdata_by_custid(hanger, 7, '195')
        if HangerSize == '6"':
            set_parameter_by_name(hanger, 'FP_Pointload', 358)
            set_customdata_by_custid(hanger, 7, '358')
        if HangerSize == '8"':
            set_parameter_by_name(hanger, 'FP_Pointload', 503)
            set_customdata_by_custid(hanger, 7, '503')
    except:
